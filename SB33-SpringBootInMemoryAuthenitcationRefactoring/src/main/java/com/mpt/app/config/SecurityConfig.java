...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable().authorizeRequests().antMatchers("/api/tickets/ticket/**").
		hasAnyRole("user","admin").and().authorizeRequests().antMatchers("/api/tickets/admin/**").
		hasAnyRole("admin").and().formLogin();
	}
	...
	public void configureGlobalBuilder(AuthenticationManagerBuilder auth) throws Exception {
		...
		// Refactoring
		auth.inMemoryAuthentication().withUser("Jeet").password("{noop}123").roles("user").and().
		withUser("Jitu").password("{noop}1234").roles("user","admin");
	}
}
